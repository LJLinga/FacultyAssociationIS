<html>
	...
	<header>
		...
		<title>Manage Member Applications - Faculty Association</title>
	...
	<?php 
		...
		$queryMem = "SELECT M.MEMBER_ID, M.LASTNAME, M.FIRSTNAME, M.MIDDLENAME, M.DATE_APPLIED, D.DEPT_NAME, P.PAYROLL_NAME FROM MEMBER AS M
					 JOIN REF_DEPARTMENT AS D
					 ON M.DEPT_ID = D.DEPT_ID
					 JOIN REF_PAYROLL AS P
					 ON M.PAYROLL_ID = P.PAYROLL_ID
					 WHERE USER_STATUS_ID = '1' AND APP_STATUS_ID = '1'";
		...
		if (isset($_POST['approve']) && !empty($_POST['selected'])) {
			...
			if (isset($_POST['selected'])) {
				...
				foreach ($_POST['selected'] as $selected) {
					...
					$queryAcceptMem = "UPDATE MEMBER SET APP_STATUS_ID = 2, EMP_ID_APPROVE = '{$empid}', DATE_ACCEPTED = NOW() WHERE MEMBER_ID = '{$selected}'";
					...
					$queryTransaction = "INSERT INTO TRANSACTIONS (MEMBER_ID, AMOUNT, TXN_DATE, TXN_TYPE, TXN_STATUS, EMP_ID_INVOLVED) 
								 VALUES ({$selected}, 0, NOW(), 1, 'Member Application Approved', {$empid})";
		...
		if (isset($_POST['reject']) && !empty($_POST['selected'])) {
			...
			if (isset($_POST['selected'])) {
				...
				foreach ($_POST['selected'] as $selected) {
					...
					$queryRejectMem = "UPDATE MEMBER SET APP_STATUS_ID = 3 WHERE MEMBER_ID = '{$selected}'";

					$resultRejectMem = mysqli_query($dbc, $queryRejectMem);

					$queryTransaction = "INSERT INTO TRANSACTIONS (MEMBER_ID, AMOUNT, TXN_DATE, TXN_TYPE, TXN_STATUS, EMP_ID_INVOLVED) 
								 VALUES ({$selected}, 0, NOW(), 1, 'Member Application Rejected', {$empid})";
		...
		if (isset($_POST['viewdetails']) && !empty($_POST['viewdetailscheck'])) {
			...
			header("Location: http://".$_SERVER['HTTP_HOST'].  dirname($_SERVER['PHP_SELF'])."/admin memberapplication_viewdetails.php");
	...
	<body>
		...
		<h1>Pending Member Applications</h1><p>
