<html>
	...
	<?php 
		...
		$queryMem = "SELECT ML.RECORD_ID, ML.DATE_APPLIED, ML.MEMBER_ID, M.LASTNAME, M.FIRSTNAME, D.DEPT_NAME, P.PAYROLL_NAME, B.BANK_ABBV, L.AMOUNT
			...
			if (isset($_POST['selected'])) {
				...
				foreach ($_POST['selected'] as $selected) {
					...
					$queryTransaction = "INSERT INTO TRANSACTIONS (MEMBER_ID, AMOUNT, TXN_DATE, TXN_TYPE, TXN_STATUS, EMP_ID_INVOLVED) 
								 VALUES ({$rowRecord['MEMBER_ID']}, 0, NOW(), 1, 'Bank Loan Application Approved', {$empid})";
		...
		if (isset($_POST['reject']) && !empty($_POST['selected'])) {
			...
			if (isset($_POST['selected'])) {
				...
				foreach ($_POST['selected'] as $selected) {
					...
					$queryTransaction = "INSERT INTO TRANSACTIONS (MEMBER_ID, AMOUNT, TXN_DATE, TXN_TYPE, TXN_STATUS, EMP_ID_INVOLVED) 
								 VALUES ({$rowRecord['MEMBER_ID']}, 0, NOW(), 1, 'Bank Loan Application Rejected', {$empid})";
		...
		if (isset($_POST['viewdetails']) && !empty($_POST['viewdetailscheck'])) {
			...
			header("Location: http://".$_SERVER['HTTP_HOST'].  dirname($_SERVER['PHP_SELF'])."/admin bankloan_viewdetails.php");
